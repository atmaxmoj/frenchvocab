#!/usr/bin/env python3
"""校验 catchup batch (46 mots) + 合入 skeleton / enrich_out / etym_hook_out / formF_out."""
...
# load bundles
bundles = []
for fn in sorted(glob.glob("enrich_stage/enr_catchup_*.json")):
    bundles += json.load(open(fn, encoding="utf-8"))
print(f"loaded {len(bundles)} bundles from {len(glob.glob('enrich_stage/enr_catchup_*.json'))} files")

seed = {w["id"]: w for w in json.load(open("seed_catchup.json"))}
